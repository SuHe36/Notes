...
def SGD_FM(dataMatrix, classLabels, k, iter):
    ...
    for it in range(iter):
        for x in range(m):  # 随机优化，每次只使用一个样本
            ...
            # 这个dataMatrix[x]的维度是[1,8]， v的维度是[8,20], inter1的维度是[1, 20]
            # ataMatrix[x]就是一个样本的特征值，然后去v中取对应下标的20维向量，然后相乘相加

            inter_2 = multiply(dataMatrix[x], dataMatrix[x]) * multiply(v, v)  #二阶交叉项的计算
            # dataMatrtix[x]的维度是[1,8], v的维度是[8,20]，
            # 其中multiply(dataMatrix[x], dataMatrix[x])的结果维度是[1,8]，他执行的结果也就是两个向量对位相乘
            # 相应的multiply(v, v)的结果维度也是[8,20]
            # 而2最终inter_2的维度是[1,20]

            interaction = np.sum(multiply(inter_1, inter_1) - inter_2) / 2.       #二阶交叉项计算完成
            # 最终np.sum()括号内的结果的维度为[1,20]
            # 最终interaction是一个数值，比如为20.0，
            # 这个interaction最终表示的是w_{ij}x_ix_j的结果值
...
if __name__ == '__main__':
    ...
    print    ("开始训练")
    w_0, w, v = SGD_FM(mat(dataTrain), labelTrain, 20, 10)
    print(
