 ...
 - 设$f*g = h(z) = \int_{\mathbb{R}} f(x)g(z-x)dx$
  ...
  $$\begin{aligned}
      F\{f*g\} =& F\{h(v)\} \\
               ...
               =& \int_{\mathbb{R}}f(x)(\int_{\mathbb{R}}f(x)g(z-x)e^{-iwz}dz)dx
  \end{aligned} $$

    代入$y=z-x, dy=dz$可以得到：
    $$
    \begin{aligned}
    F\{f*g\} =& \int_{\mathbb{R}}f(x)(\int_{\mathbb{R}}f(x)g(z-x)e^{-iwz}dz)dx \\
         =& \int_{\mathbb{R}}f(x)(\int_{\mathbb{R}}f(x)g(z-x)e^{-iw(x+y)}dy)dx \\
         =& \int_{\mathbb{R}}f(x)e^{-iwx}dx \int_{\mathbb{R}}f(y)e^{-iwy}dy \\
         =& F\{f\} \cdot F\{g\}
    \end{aligned}
    $$
    最后两边同时作用$F^{-1}$，可以得到：
